...
function get_users_data {
    ...
    foreach ($user in $users) {
        $MFA_method = if ($user.StrongAuthenticationMethods) { $user.StrongAuthenticationMethods | Where-Object {$_.IsDefault -eq $true} | Select-Object -ExpandProperty MethodType } else { "None" }
...
function get_OU_users {
    ...
    try { return Get-ADUser -SearchBase $ouDN -Filter * | Select-Object -ExpandProperty UserPrincipalName } catch { write-host "Error: $_" -ForegroundColor Red }
}
...
function get_AD_users {
    ...
    foreach ($ouDN in $ouDNs) {
        if (!($OU_users = get_OU_users -ouDN $ouDN)) { continue }
        
        $users = get_users_data -OU_users $OU_users
        export_csv -csv_file_path "$PSScriptRoot\_logs\exported\MFA users status - $ouDN $(Get-Date -Format 'dd-MM-yyyy h.mmtt').csv" -data $users
    }
}
