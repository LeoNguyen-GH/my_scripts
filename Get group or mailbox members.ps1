...
function get_group_members {
    ...
    try {
        return $members = switch (get_group_type -email $group) {
            ...
            { $_ -match "Mailbox" } { Get-MailboxPermission -Identity (Get-Mailbox -Identity $group) | Where-object { $_.User -notlike "*SELF*" } | Select-Object @{Name='Name'; Expression={$_.User}},AccessRights }
...
function main {
    ...
    foreach ($group in $groups) {
        if ($members = get_group_members -group $group) {
            ...
            Write-Host "No members found for '$group'."
        }
    }
}
